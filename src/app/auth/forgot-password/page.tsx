...
export default function ForgotPasswordPage() {
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 selection:bg-primary selection:text-primary-foreground relative overflow-hidden">
            ...
            <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-blue-500/20 blur-[128px] rounded-full pointer-events-none" />
                    ...
                    <CardContent className="px-8 pb-10">
                        {state?.success ? (
                            <motion.div
                                ...
                                className="p-4 bg-green-500/10 border border-green-500/20 text-green-600 rounded-xl text-sm font-medium flex flex-col items-center justify-center text-center gap-3 py-8"
                            >
                                <CheckCircle2 className="w-12 h-12 mb-2" />
                                <p>{state.message || 'Check your email for the password reset link.'}</p>
                                <Link href="/auth/login" className="mt-4 text-primary font-bold hover:underline">
                                    Return to login
                                </Link>
                            </motion.div>
                        ) : (
